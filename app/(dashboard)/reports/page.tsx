...
import {
  ...
  Download, ArrowUpRight, Target, PieChart, Activity, Clock
} from "lucide-react";
import { format, startOfMonth, endOfMonth, subMonths } from "date-fns";

// ── Skeletons ──────────────────────────────────────────────
function SkeletonHighlight() {
  return <div className="rounded-3xl p-8 bg-gray-100 animate-pulse h-48" />;
}

function SkeletonCard() {
  return (
    <div className="rounded-3xl p-6 bg-white border border-gray-100 animate-pulse space-y-4 h-64">
...
export default function ReportsPage() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && Object.keys(response.data).length > 0) {
        ...
        setStats(null);
        showToast("No data returned from server", "error");
      }
    } catch (error: any) {
  ...
  }, [monthYear, timeRange, getStartDate]);
  ...
  const collectionRate = stats
    ? Math.min((stats.totalPaid / (stats.totalRevenue || 1)) * 100, 100)
    : 0;
  const completionRate = stats
    ? Math.min((stats.appointmentsCompleted / (stats.totalAppointments || 1)) * 100, 100)
    : 0;
  ...
  const generateCSV = useCallback(() => {
    ...
    const rows = [
      ...
      ["Pending Dues",       `₹${stats.pendingDues ?? 0}`],
      ["Collection Rate",    `${collectionRate.toFixed(2)}%`],
      ["Avg Bill Value",     `₹${Math.round((stats.totalRevenue ?? 0) / (stats.totalBills || 1))}`],
      [""],
      ["CLIENT METRICS"],
      ["Total Clients",      stats.totalClients ?? 0],
      ["New Clients",        stats.newClients ?? 0],
      ["Active Clients",     stats.activeClients ?? 0],
      ["Inactive Clients",   stats.inactiveClients ?? 0],
      ["Discharged Clients", stats.dischargedClients ?? 0],
      [""],
      ["APPOINTMENT ANALYTICS"],
      ["Total Appointments",     stats.totalAppointments ?? 0],
      ["Completed",              stats.appointmentsCompleted ?? 0],
      ["No-Show",                stats.appointmentsNoShow ?? 0],
      ["Cancelled",              stats.appointmentsCancelled ?? 0],
      ["Completion Rate",        `${completionRate.toFixed(2)}%`],
      ["Avg Session Duration",   `${Math.round(stats.avgSessionDuration ?? 0)} mins`],
      ["Clients Improving",      stats.sessionsImproving ?? 0],
    ];
    return rows.map((r) => r.join(",")).join("\n");
  }, [stats, collectionRate, completionRate, getStartDate, monthYear]);
  ...
  const handleExport = useCallback(() => {
    ...
    link.setAttribute("download", `ClinDesk_Analytics_${format(monthYear, "MMM_yyyy")}.csv`);
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  }, [generateCSV, monthYear]);
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex items-end justify-between flex-wrap gap-6 relative">
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-gray-900 hover:bg-gray-800 text-white rounded-xl font-semibold transition-all hover:shadow-xl hover:-translate-y-0.5 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <Download size={18} />
          Export Data
        </button>
      ...
      <div className="bg-white/80 backdrop-blur-md border border-gray-200 p-2 rounded-2xl sticky top-4 z-30 shadow-sm flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-xl border border-gray-100 flex-1 min-w-[200px]">
          ...
          <input
            ...
            onChange={(e) => {
              const parsed = new Date(e.target.value + "-01");
              if (!isNaN(parsed.getTime())) setMonthYear(parsed);
            }}
          ...
          <>
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-6">
                  <div className="p-3 bg-orange-50 rounded-2xl text-orange-600"><Target size={24} /></div>
                ...
                <p className="text-3xl font-black text-gray-900">₹{(stats.pendingDues ?? 0).toLocaleString("en-IN")}</p>
              </div>
              <p className="text-sm text-gray-400 mt-4">
                Uncollected from {stats.pendingBills ?? 0} pending invoices.
              </p>
            </div>
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
              ...
              <div className="mt-4 flex items-center gap-1.5 text-green-600 font-bold text-sm">
                ...
                <span>Growth: {(((stats.newClients ?? 0) / (stats.totalClients || 1)) * 100).toFixed(1)}%</span>
                <span className="text-gray-300 font-normal ml-auto text-xs">of {stats.totalClients ?? 0} total</span>
              </div>
            </div>
          </>
        )}
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card className="p-0 overflow-hidden border-none shadow-xl bg-slate-50">
              <div className="p-6 border-b border-gray-200 bg-white">
                <h3 className="font-bold text-gray-900 flex items-center gap-2">
                  <Users size={20} className="text-blue-600" /> Patient Status
                </h3>
              </div>
              <div className="p-6 grid grid-cols-2 gap-4">
                {[
                  { label: "Ongoing Care",     value: stats.activeClients ?? 0,    color: "text-teal-600",   sub: `${(((stats.activeClients ?? 0) / (stats.totalClients || 1)) * 100).toFixed(0)}%` },
                  { label: "Total Recovered",  value: stats.dischargedClients ?? 0, color: "text-blue-600",  sub: "Treatment Finished" },
                  { label: "Inactive",         value: stats.inactiveClients ?? 0,   color: "text-orange-500", sub: "Dormant" },
                  { label: "Lifetime Total",   value: stats.totalClients ?? 0,      color: "text-gray-900",  sub: "All time" },
                ].map((item) => (
            ...
            <Card className="p-0 overflow-hidden border-none shadow-xl bg-slate-50">
              <div className="p-6 border-b border-gray-200 bg-white">
                <h3 className="font-bold text-gray-900 flex items-center gap-2">
                  <Calendar size={20} className="text-purple-600" /> Appointment Stats
                </h3>
              </div>
              <div className="p-6 space-y-4">
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="text-center">
                      <p className="text-xl font-black text-gray-900">{stats.appointmentsCompleted ?? 0}</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Success</p>
                    </div>
                    <div className="text-center border-x border-gray-100">
                      <p className="text-xl font-black text-gray-900">{stats.appointmentsNoShow ?? 0}</p>
                      <p className="text-[10px] font-bold text-red-400 uppercase">No-Show</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xl font-black text-gray-900">{stats.appointmentsCancelled ?? 0}</p>
                      <p className="text-[10px] font-bold text-amber-500 uppercase">Cancelled</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <div className="flex-1 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                    <div className="p-3 bg-gray-50 rounded-xl text-gray-600"><Clock size={20} /></div>
                    <div>
                  ...
                  <div className="flex-1 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                    <div className="p-3 bg-green-50 rounded-xl text-green-600"><TrendingUp size={20} /></div>
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Recovery Rate</p>
                      <p className="text-lg font-black text-gray-900">
                        {stats.sessionsImproving ?? 0}{" "}
                        <span className="text-xs font-normal text-green-500">Improving</span>
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm leading-relaxed mt-1">
                        ...
                        Targeting 90%+ could unlock ₹{Math.round((stats.pendingDues ?? 0) * 0.5).toLocaleString("en-IN")} in cash flow.
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm leading-relaxed mt-1">
                        ...
                        <span className="text-red-400 font-bold">
                          {(((stats.appointmentsNoShow ?? 0) / (stats.totalAppointments || 1)) * 100).toFixed(1)}%
                        </span>. Consider SMS reminders to recover lost slots.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white/10 rounded-3xl p-8 backdrop-blur-xl border border-white/10 self-center">
                ...
                <p className="text-3xl font-black text-white py-4">
                  ₹{Math.round((stats.totalRevenue ?? 0) / (stats.totalClients || 1)).toLocaleString("en-IN")}
